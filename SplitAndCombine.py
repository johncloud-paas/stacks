...
class SplitAndCombineFiles:
    ...
    def get_file_chunks_from_count(self):
        ...
        msg(
            "Splitting into {} files of {} size".format(str(self.__chunk), str(f_chunk))
        )
    ...
    def get_file_count_from_size(self):
        ...
        if no_of_files > 1:
            no_of_files = (
                int(no_of_files) + 1
                if no_of_files % int(no_of_files) > 0
        ...
        msg(
            "Splitting into {} files of {} size".format(
                str(no_of_files), str(self.__chunk)
            )
        )
    ...
    def __split(self, input_file_name, chunk_size):
        ...
        for i in range(int(chunk_size)):
            _chunk_file_name = "{}-{}{}".format(
                str(self.__input_file_name), str(i + 1), str(self.__postfix)
            )
            ...
            self.check_list += (
                str(data[:5])
                + str(data[int(data_length / 2) - 1 : int(data_length / 2) + 1])
                + str(data[-5:])
            )
            ...
            with open(_chunk_file_name, "wb") as _:
                _.write(base64.b64encode(data))

        _crc_file_name = "{}-{}{}".format(
            str(self.__input_file_name), "CRC", str(self.__postfix)
        )

        msg("Creating the check file : {}".format(str(_crc_file_name)))
        with open(_crc_file_name, "wb") as crc_file:
            crc_file.write(self.check_list.encode())
    ...
    def __merge(self, input_file_name):
        ...
        if os.path.exists(_file_path):
            msg(
                "File Already Exist. Please remove the {} and "
        ...
        _crc_data = open(_crc_file_path, "rb").read()
